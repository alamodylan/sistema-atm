...
def generate_transfer_pdf(transfer):
    ...
    if os.path.exists(logo_path):
        logo = Image(logo_path, width=4.2 * cm, height=2.0 * cm)
    else:
        logo = Paragraph("<b>ATM</b>", styles["Heading2"])
    ...
    story.append(Paragraph("COMPROBANTE DE TRASLADO", title_style))

    sender_user_id = _get_sender_user_id(transfer)
    sender_name = _user_name(sender_user_id)
    receiver_name = _user_name(transfer.received_by_user_id)

    info_data = [
        ["Número de traslado", _text(transfer.number)],
        ["Estado", _text(transfer.status)],
        ["Fecha de creación", _text(transfer.created_at)],
        ["Fecha de envío", _text(transfer.sent_at)],
        ["Fecha de recepción", _text(transfer.received_at)],
        ["Predio origen", _text(transfer.origin_site.name if transfer.origin_site else "")],
        ["Bodega origen", _text(transfer.origin_warehouse.name if transfer.origin_warehouse else "")],
        ["Predio destino", _text(transfer.destination_site.name if transfer.destination_site else "")],
        ["Bodega destino", _text(transfer.destination_warehouse.name if transfer.destination_warehouse else "")],
    ...
    story.append(Paragraph("<b>Detalle de artículos trasladados</b>", styles["Heading3"]))
    ...
    for line in transfer.lines:
        ...
        lines_data.append([
            ...
            Paragraph(_text(article.name if article else ""), normal_style),
            _text(line.quantity_sent),
            _text(line.quantity_received if line.quantity_received is not None else ""),
    ...
    signature_data = [
        [
            Paragraph("<b>Firma de quien envía</b>", normal_style),
            Paragraph("<b>Firma de quien recibe</b>", normal_style),
        ],
        [
            "",
            "",
